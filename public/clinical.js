...
async function load(){
 ...
 if($('cycleCount'))$('cycleCount').textContent=c.length;
 if($('cycleHistoryCount'))$('cycleHistoryCount').textContent=c.length+' logged';
 if($('cycleList'))$('cycleList').innerHTML=c.length?c.map(v=>'<div class="cycle-history-row"><div><strong>'+v.cycle_start+'</strong><span>'+(v.cycle_end?' → '+v.cycle_end:' · ongoing')+'</span></div><em>'+(v.flow||'Flow not recorded')+'</em></div>').join(''):'<div class="cycle-empty">No previous cycles yet.</div>';
 if($('cycleStatus'))$('cycleStatus').textContent=c.length?'Latest cycle':'No cycle yet';
 if($('cycleOverview'))$('cycleOverview').innerHTML=c.length?'<div class="cycle-stat"><strong>'+c[0].cycle_start+'</strong><span>latest start</span></div><div class="cycle-stat"><strong>'+(c[0].cycle_end||'Ongoing')+'</strong><span>end date</span></div><div class="cycle-stat"><strong>'+(c[0].flow||'—')+'</strong><span>flow</span></div>':'<div class="cycle-empty">Log your first cycle below to start building your history.</div>';
 if($('menstrualEventList'))$('menstrualEventList').innerHTML=e.length?e.slice(0,20).map(v=>'<span class="event-chip"><b>'+v.event_date+'</b> '+String(v.event_type).replaceAll('_',' ')+'</span>').join(''):'<div class="cycle-empty">No events logged yet.</div>';
